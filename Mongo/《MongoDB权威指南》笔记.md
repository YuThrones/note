...
* MapReduce有几个步骤，最开始是映射(map)，将操作映射到集合中的每个文档，这个从操作要么“无作为”，要么“产生一些键和X个值”。然后是中间环节，称作洗牌（shuffle），按照键分组，并将产生的键值组成列表放到对应的键中。化简(reduce)这把列表中的值化简为一个单值并返回，接着进行洗牌，知道每个键的列表都只有一个值，也就是最后结果。
...
* 用shell连接mongos之后可以通过addshard命令添加片
