...
* TCP通过维持一个称为 **阈值（Threshold）** 的变量来管理这些复杂过程。它是用来确定慢启动即将结束并且拥塞避免将开始的窗口长度。初始时会设的很大值以使它没有初始效应。每当出现一个丢包事件，Threshold就被设为当前Congwin值的一半。在慢启动阶段，Congwin值指数增长，直至达到Threshold值，就进入拥塞避免阶段。
